...
public final class Deviation {
	...
	private MotionBook mMotionBook;
	...
	public Deviation(JSONObject json) throws JSONException {
		...
		if (json.has("is_downloadable")) {
			mIsDownloadable = json.getBoolean("is_downloadable");
		}
		mIsDeleted = json.getBoolean("is_deleted");
		if (json.has("is_favourited")) {
			mIsFavourited = json.getBoolean("is_favourited");
		}
		if (json.has("is_mature")) {
			mIsMature = json.getBoolean("is_mature");
		}
		if (json.has("motion_book")) {
			mMotionBook = new MotionBook(json.getJSONObject("motion_book"));
		}
		if (json.has("preview")) {
			mPreview = new Image(json.getJSONObject("preview"));
		}
		mPrintId = json.optString("printid");
		if (json.has("published_time")) {
	...
	public final MotionBook getMotionBook() {
		return mMotionBook;
	}
	...
	public final class MotionBook {
		private String mEmbedUrl;
		
		public MotionBook(JSONObject json) throws JSONException {
			mEmbedUrl = json.getString("embed_url");
		}
