...
public class DiscordBot extends ListenerAdapter {
	...
	public static void main(String[] args) throws GeneralSecurityException, IOException {
		...
		try {
			...
			e.printStackTrace();
			System.out.println("We were unable to connect to the MySQL Server! Please check your Settings!");
			System.exit(0);
		}
		...
		if (cmd.hasOption("ext")) {
			...
			HashMap<String, ChatLogChannel> c = CommandExecutor.getChatLog()
					.listChannels(bot.getGuildsByName(guild, true).get(0));
			int msgs = 0;
	...
	private static CommandLine parseArguments(String[] args) {
		...
		Option discordToken = OptionBuilder.withLongOpt("discordToken").withArgName("token")
				.withDescription("The Discord Token of the Bot instance").hasArg().isRequired().create("dT");
		Option extractChatLog = OptionBuilder.withLongOpt("extractChat")
				.withDescription("Extracts the ChatLog of a given Discord Server").withArgName("server").hasArg()
				.create("ext");
		Option extractChannel = OptionBuilder.withLongOpt("extractChannel")
				.withDescription("Extracts only a single Channel").withArgName("channel").hasArg().create("exChan");

		Option gameDisplay = OptionBuilder.withLongOpt("displayedGame").withArgName("game").hasArg()
				.withDescription("Displays given Game as currently playing (Default: Rule34)").create("game");
		Option threads = OptionBuilder.withArgName("threadCount").hasArg()
				.withDescription("Sets the Threads used by the Bot to reply to commands (Default: 5)")
				.create("threads");
		Option mysql = OptionBuilder.withArgName("ip:port").hasArg()
				.withDescription("Sets ip and port of the used MySQL Server (Default: localhost:3306)")
				.create("mysqlIP");
		Option sqluser = OptionBuilder.isRequired().withArgName("user").hasArg()
				.withDescription("Sets the user used for the MySQL Server").create("sqlUser");
		Option sqlPW = OptionBuilder.isRequired().withArgName("pw").hasArg()
				.withDescription("Sets the password used for the MySQL Server").create("sqlPassword");
		Option sqlDB = OptionBuilder.isRequired().withArgName("db").hasArg()
	...
	public static void registerEmoteChangeListener(Message m, ListenerAdapter la, long time) {
		Cleanable<ListenerAdapter> l = new Cleanable<ListenerAdapter>(la, time);
		l.addToMap(m.getId(), registeredListeners);
	}
	
	static HashMap<String, Cleanable<ListenerAdapter>> registeredListeners = new HashMap<>();
	...
	public void onGuildMessageReactionAdd(GuildMessageReactionAddEvent gmrae) {
		if(registeredListeners.containsKey(gmrae.getMessageId())) {
			Cleanable<ListenerAdapter> l = registeredListeners.get(gmrae.getMessageId());
			ListenerAdapter la = l.getObject();
			if(la==null) {
	...
	public void onGuildMessageReactionRemove(GuildMessageReactionRemoveEvent gmrre) {
		if(registeredListeners.containsKey(gmrre.getMessageId())) {
			Cleanable<ListenerAdapter> l = registeredListeners.get(gmrre.getMessageId());
			ListenerAdapter la = l.getObject();
			if(la==null) {
